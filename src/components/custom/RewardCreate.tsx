...
export default function RewardCreate() {
  const [creatingReward, setCreatingReward] = React.useState(false);

  return !creatingReward ? (
    <motion.div
      whileHover={{ scale: 1.03 }}
      whileTap={{ scale: 0.97 }}
      className="min-w-[300px] w-min h-min max-h-[100px] m-2"
    ...
    >
      ...
      >
        <CardContent className="flex flex-col items-center justify-center space-y-3 text-center">
          ...
          <p className="text-lg font-medium tracking-wide">Create Reward</p>
        </CardContent>
      </Card>
    </motion.div>
  ) : (
    <RewardForm
      ...
      onCancel={() => setCreatingReward(false)}
    />
  );
}
